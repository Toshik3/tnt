...
def process_folder(source_folder, destination_folder):
    ...
    for root, dirs, files in os.walk(source_folder):
        ...
        for file in files:
            ...
            if file_extension != "":
                if file_extension[1:].upper() in allowed_image_extensions:
                    subdir = 'images'
                elif file_extension[1:].upper() in allowed_video_extensions:
                    subdir = 'video'
                elif file_extension[1:].upper() in allowed_doc_extensions:
                    subdir = 'documents'
                elif file_extension[1:].upper() in allowed_audio_extensions:
                    subdir = 'audio'
                elif file_extension[1:].upper() in allowed_archive_extensions:
                    subdir = 'archives'
